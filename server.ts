...
type TravelHelperResponseMode =
  | 'direct_answer'
  | 'clarify_airport'
  | 'clarify_restaurant_preferences'
  | 'clarify_activity_preferences'
  | 'clarify_missing_trip_inputs'
  | 'trip_proposal'
  | 'host_answer';
...
const AIRPORT_OPTIONS_BY_AREA: Record<string, DestinationAirportOption[]> = {
  trysil: [
    {
      code: 'OSL',
...
function getAirportRecommendationOptions(area: string | null): DestinationAirportOption[] {
...
function extractTravelHelperDates(
  ...
  const compactNumericRange = raw.match(
    /\b(\d{1,2})[.\-/](\d{1,2})(?:[.\-/](\d{2,4}))?\s*(?:til|-|–)\s*(\d{1,2})[.\-/](\d{1,2})(?:[.\-/](\d{2,4}))?\b/i
  );
  ...
  const compactSameMonth = message.match(
...
function messageMentionsAirportExplicitly(messageRaw: string): boolean {
  const message = normalizeTravelHelperText(messageRaw);
...
function needsRestaurantClarification(messageRaw: string, history: any[]): boolean {
  ...
  if (!asksForRestaurants) return false;
...
function needsActivityClarification(messageRaw: string, history: any[]): boolean {
  const combined = buildTravelHelperSearchBasis(messageRaw, history);
  const message = normalizeTravelHelperText(combined);

  const asksForActivities =
  ...
  if (!asksForActivities) return false;
...
function buildAirportClarificationReply(area: string | null): string {
  ...
  const label =
    area === 'trysil' || String(area || '').startsWith('trysil')
      ? 'Trysil'
      : area === 'salen'
        ? 'Sälen'
        : area || 'destinasjonen';
...
function buildRestaurantClarificationReply(area: string | null): string {
...
function buildActivityClarificationReply(): string {
...
function scoreSeasonalContentItem(
  ...
  if (message.includes('restaurant') || message.includes('mat')) {
    if (item?.category === 'restaurant' || item?.category === 'restaurant_place') score += 15;
  }

  if (message.includes('aktivitet') || message.includes('skiferie') || message.includes('sommerferie')) {
    if (item?.category === 'activity' || item?.category === 'activity_place') score += 15;
  }
  ...
  if (
    ...
    if (item?.category === 'host_rental') score += 40;
  }
...
app.post('/api/travel-helper', async (req, res) => {
  try {
    ...
    let responseMode: TravelHelperResponseMode = hostRentalIntent
      ? 'host_answer'
      : 'direct_answer';
    ...
    if (contentIntent || tripPlanningIntent || hostRentalIntent) {
      try {
        ...
        const destinationForContent =
          contentCategory === 'travel_terms' || contentCategory === 'host_rental'
            ? 'global'
        ...
        const prioritizedItems =
          contentCategory === 'travel_terms' && !tripPlanningIntent
...
app.post('/api/host-helper', async (req, res) => {
  try {
    ...
    try {
      ...
      const deduped = [...hostItems, ...travelTermsItems].filter(
        (item, index, arr) => arr.findIndex((x) => x?.slug === item?.slug) === index
      );
      ...
      const prioritizedItems =
        normalizedMessage.includes('innsjekk') ||
