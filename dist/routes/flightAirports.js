    ...
    try {
        ...
        if (!token) {
            return res.status(500).json({
                ...
                detail: 'Fant ikke DUFFEL_ACCESS_TOKEN i environment variables. Sjekk .env-filen eller Render.',
            });
        }
        console.log('[DUFFEL] place search', { q });
        const response = await fetch(`https://api.duffel.com/places/suggestions?query=${encodeURIComponent(q)}`, {
            method: 'GET',
            headers: {
        ...
        const data = list
            ...
            .filter((item) => !!item.code && (!!item.airport || !!item.city));
