...
function looksLikeLinen(name: string): boolean {
...
async function main() {
  ...
  if (!serviceIds.length) throw new Error('Missing MEWS_SERVICE_IDS (or MEWS_SERVICE_ID)');

  console.log('== List products per service ==');
  console.log('Services:', serviceIds.join(', '));

  const linenProductIds: string[] = [];
...
.filter((x: { id: string; name: string }) => x.id && x.name && looksLikeLinen(x.name));
