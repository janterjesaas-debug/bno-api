...
function pickName(p: any): string {
  ...
  if (n && typeof n === 'object') {
    ...
    if (k) return String(n[k] || '');
  }
  return '';
}

function parsePersonsFromName(name: string): number {
...
function buildServiceOrderToReservationIdMap(reservations: any[]): Record<string, string> {
...
async function main() {
  ...
  if (!serviceId) throw new Error('Missing --service <serviceId>');
  if (!start || !end) throw new Error('Missing --start YYYY-MM-DD and --end YYYY-MM-DD');
  ...
  const [reservations, orderItems, products, resources] = await Promise.all([
    fetchReservationsForCleaningRange(serviceId, start, end),
    fetchOrderItemsForCleaningRange(serviceId, start, end),
  ...
  for (const r of resources || []) {
    if (r?.Id && r?.Name) resIdToName[String(r.Id)] = String(r.Name);
  }
  ...
  if (envIds.length) {
    ...
    for (const p of products || []) {
      if (p?.Id) byId[String(p.Id).toLowerCase()] = p;
    }
  ...
  const linenCountByReservationId = buildLinenCountMapFromOrderItems(orderItems, linenProducts, soToRes);

  const rows = (reservations || []).map((r: any) => {
    const rid = String(r.Id);
    const assignedResourceId = r.AssignedResourceId ? String(r.AssignedResourceId) : '';
    const unitName =
      (r.AssignedResourceId && resIdToName[String(r.AssignedResourceId)]) ||
