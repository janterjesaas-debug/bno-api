...
import { fetchReservationsForCleaningRange } from '../../lib/mews';
...
function requireApiKey(req: Request) {
  ...
  if (authDebug) {
    const mask = (s: string) =>
      s && s.length >= 10 ? `${s.slice(0, 6)}...${s.slice(-4)}` : s ? `[len=${s.length}]` : '';
    console.log(
      '[AUTH DEBUG]',
      'expectedLen=',
      expected.length,
  ...
  if (!incoming || incoming !== expected) {
    const e: any = new Error(!incoming ? 'Mangler API-nøkkel.' : 'Ugyldig API-nøkkel.');
    e.status = 401;
    e.body = {
      ok: false,
...
function parseServiceId(req: Request): string | null {
  // 1) query override
...
function pickDateOnly(value: any): string {
...
function sortByScheduledEndUtc(rows: any[]): any[] {
...
async function handler(req: Request, res: Response) {
  try {
    ...
    const rowsRaw = await fetchReservationsForCleaningRange(serviceId, from, to);
    ...
    if (mews) out.mews = mews;

    return res.status(status).json(out);
  }
}

export function registerMewsReservationsRoute(app: Express) {
  console.log(
    `[BOOT] ${BOOT_TAG} registering routes: GET /mews/reservations and GET /api/mews/reservations`,
  );
