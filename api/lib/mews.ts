...
/**
 * Kaller Mews Connector API → availability for «Stay»-service.
 */
export async function fetchMewsAvailability(req: AvailabilityRequest) {
  ...
  // Viktig: korrekt sti er /services/availability
  const url = `${BASE}/services/availability`;
  ...
  if (!res.ok) throw new Error(`Mews ${res.status} ${res.statusText}: ${text.slice(0, 800)}`);
  try {
