...
exports.toTimeUnitUtc = toTimeUnitUtc;
exports.parseIsoDurationToMs = parseIsoDurationToMs;
exports.fetchAvailability = fetchAvailability;
exports.fetchCategoriesRaw = fetchCategoriesRaw;
exports.fetchImageUrls = fetchImageUrls;
exports.fetchProducts = fetchProducts;
exports.fetchService = fetchService;
exports.fetchAvailabilityNamed = fetchAvailabilityNamed;
exports.findOrCreateCustomer = findOrCreateCustomer;
exports.createReservation = createReservation;
exports.fetchReservations = fetchReservations;
exports.createProductServiceOrders = createProductServiceOrders;
exports.fetchResources = fetchResources;
exports.fetchSpaces = fetchResources;
exports.fetchOrderItemsForCleaningRange = fetchOrderItemsForCleaningRange;
exports.fetchReservationsForCleaningRange = fetchReservationsForCleaningRange;
exports.buildLinenCountMapFromOrderItems = buildLinenCountMapFromOrderItems;
// bno-api/lib/mews.ts
...
function assertDefaultCreds() {
    if (!baseUrlDefault)
        throw new Error('Missing MEWS_BASE_URL');
    if (!clientTokenDefault)
        throw new Error('Missing MEWS_CLIENT_TOKEN');
    if (!accessTokenDefault)
        throw new Error('Missing MEWS_ACCESS_TOKEN');
}
function isStrandaService(serviceId) {
    if (!serviceId)
        return false;
    const sid = String(serviceId).trim().toLowerCase();
    return strandaServiceIds.some((x) => x.trim().toLowerCase() === sid);
}
function getCredsForService(serviceId) {
    if (isStrandaService(serviceId)) {
        ...
        if (!strandaBaseUrl)
            throw new Error('Missing MEWS_STRANDA_BASE_URL (or MEWS_BASE_URL as fallback)');
        if (!strandaClientToken)
            throw new Error('Missing MEWS_STRANDA_CLIENT_TOKEN (or MEWS_CLIENT_TOKEN as fallback)');
        return {
            key: 'STRANDA',
...
// ---------- helpers ----------
function tzOffsetMinutesAt(utcInstant, tz) {
    try {
...
function toTimeUnitUtc(date) {
    ...
    let candidate = new Date(utcMidnight.getTime() - off * 60000);
    const off2 = tzOffsetMinutesAt(candidate, hotelTimeZone);
    if (off2 !== off)
        candidate = new Date(utcMidnight.getTime() - off2 * 60000);
...
function removeUndefinedRecursive(obj) {
    ...
    if (typeof obj === 'object') {
        ...
        for (const k of Object.keys(obj)) {
            const rv = removeUndefinedRecursive(obj[k]);
            if (rv !== undefined)
                out[k] = rv;
        }
...
function sanitizeName(v) {
    ...
    if (s.replace(/[^A-Za-z]/g, '').length < 2)
        s = 'Guest';
    return s.substring(0, 64);
}
/** Enkel ISO8601 duration-parser (PnDTnHnMnS -> ms) */
function parseIsoDurationToMs(dur) {
    if (!dur)
        return null;
...
async function postJson(serviceIdForCreds, url, body, timeout = 15000) {
    ...
    try {
        ...
        try {
            const safe = { ...(cleaned || {}) };
            if ('ClientToken' in safe)
                safe.ClientToken = '[MASKED]';
            if ('AccessToken' in safe)
                safe.AccessToken = '[MASKED]';
            if ('Images' in safe)
                safe.Images = '[omitted]';
            console.log('MEWS CALL', `[${creds.key}]`, url, Object.keys(safe || {}).length ? JSON.stringify(safe).slice(0, 1000) : '{}');
        }
        catch { }
...
// ---------- Connector API ----------
async function fetchAvailability(serviceId, first, last) {
...
async function fetchCategoriesRaw(serviceId) {
    const creds = getCredsForService(serviceId);
...
async function fetchImageUrls(imageIds, serviceIdForCreds) {
    if (!imageIds?.length)
        return {};
    const creds = getCredsForService(serviceIdForCreds);
    ...
    const out = {};
    (resp.data?.ImageUrls || []).forEach((it) => {
        if (it?.ImageId && it?.Url)
            out[it.ImageId] = it.Url;
    });
    return out;
}
...
async function fetchAvailabilityNamed(serviceId, first, last) {
    const availability = await fetchAvailability(serviceId, first, last);
    const timeUnits = availability?.TimeUnitStartsUtc ?? availability?.DatesUtc ?? [];
    let rcAvail = availability?.ResourceCategoryAvailabilities;
    ...
    const categories = await fetchCategoriesRaw(serviceId);
    const catMap = {};
    categories.forEach((c) => {
        catMap[c.Id] = c;
    });
    const categoryIds = rcAvail.map((r) => r.ResourceCategoryId).filter(Boolean);
    const catImageIds = {};
    if (categoryIds.length) {
        ...
        try {
            ...
            const assigns = resp.data?.ResourceCategoryImageAssignments || [];
            assigns.forEach((a) => {
                if (a?.IsActive && a?.CategoryId && a?.ImageId) {
                    if (!catImageIds[a.CategoryId])
                        catImageIds[a.CategoryId] = [];
...
// ---------- Customers ----------
async function findOrCreateCustomer(customer, serviceIdForCreds) {
    if (!customer)
        throw new Error('Missing customer');
    // try to infer creds from payload if not provided
    const inferredServiceId = serviceIdForCreds ||
        customer?.serviceId ||
        customer?.ServiceId ||
        customer?.serviceID ||
        customer?.ServiceID ||
    ...
    try {
        ...
        const resp = await postJson(inferredServiceId, urlGetAll, {
            ClientToken: creds.clientToken,
            AccessToken: creds.accessToken,
            Client: creds.clientName,
            Emails: [email],
        ...
        const found = Array.isArray(resp?.data?.Customers)
            ? resp.data.Customers.find((c) => String(c?.Email || '').toLowerCase() === email)
            : null;
        if (found?.Id)
            return found.Id;
    }
    catch (err) { }
    async function tryAdd(firstName, lastName) {
        ...
        const resp = await postJson(inferredServiceId, urlAdd, addBody, 15000);
        const id = (Array.isArray(resp?.data?.CustomerIds) && resp.data.CustomerIds[0]) ||
...
async function createReservation(p) {
    if (!p)
        throw new Error('createReservation: missing payload');
    const serviceId = p.ServiceId || p.serviceId || undefined;
    const creds = getCredsForService(serviceId);
    try {
        ...
        const personCounts = (firstRoom.Occupancy || []).map((o) => ({
        ...
        const cleaned = removeUndefinedRecursive(body);
        const resp = await postJson(serviceId, url, cleaned, 20000);
        if (resp?.data)
            return resp.data;
 ...
 */
async function fetchReservations(serviceId, from, to) {
    if (!serviceId)
        throw new Error('fetchReservations: missing serviceId');
    const creds = getCredsForService(serviceId);
    ...
    const resp = await postJson(serviceId, url, removeUndefinedRecursive(body), 20000);
    return resp.data || {};
}
/** Create product service orders (attach ordered products to reservation) */
async function createProductServiceOrders(serviceId, reservationId, orders) {
    if (!serviceId)
 ...
 */
async function fetchOrderItemsForCleaningRange(serviceId, startDate, endDate) {
    const serviceIds = Array.isArray(serviceId)
        ? serviceId.map((s) => String(s).trim()).filter(Boolean)
        : [String(serviceId).trim()].filter(Boolean);
    if (!serviceIds.length)
        throw new Error('fetchOrderItemsForCleaningRange: missing serviceId(s)');
    const keys = new Set(serviceIds.map((sid) => (isStrandaService(sid) ? 'STRANDA' : 'DEFAULT')));
    if (keys.size > 1) {
    ...
    async function fetchIntervalParts(intervalStartMs, intervalEndMs, parts = 1, maxParts = 24) {
        ...
        for (let p = 0; p < parts; p++) {
            ...
            try {
                await sleep(150);
                const resp = await postJson(serviceIds[0], url, removeUndefinedRecursive(body), 20000);
                const data = resp.data || {};
                const pageItems = Array.isArray(data.OrderItems)
                    ? data.OrderItems
                ...
                const isLimitationError = msg.includes('limitation') ||
                    msg.includes('limitation count') ||
                    msg.includes('must be in range') ||
                    msg.includes('out of range');
                if (isLimitationError && parts * 2 <= maxParts) {
                    console.warn(`fetchOrderItemsForCleaningRange: Limitation error for chunk ${subStart.toISOString()} -> ${subEnd.toISOString()}. Subdividing (${parts * 2} parts).`);
                    await fetchIntervalParts(subStartMs, subEndMs, 2, maxParts);
                }
                else if (isLimitationError && parts * 2 > maxParts) {
                    console.error(`fetchOrderItemsForCleaningRange: Limitation error and cannot subdivide further for interval ${subStart.toISOString()} -> ${subEnd.toISOString()}`);
                }
                else {
    ...
    while (cursorYmd <= endYmd) {
        safety++;
        if (safety > safetyMax)
            throw new Error('fetchOrderItemsForCleaningRange: safety stop (too many days)');
 ...
 */
async function fetchReservationsForCleaningRangeSingle(serviceId, startTU, endExclusiveTU) {
    const creds = getCredsForService(serviceId);
    const url = `${creds.baseUrl}/api/connector/v1/reservations/getAll/2023-06-06`;
    const body = {
        ClientToken: creds.clientToken,
    ...
    const serviceOrders = Array.isArray(data.ServiceOrders)
        ? data.ServiceOrders
    ...
    for (const r of reservations) {
        ...
        const addKey = (k) => {
            ...
            if (!s)
                return;
            if (!reservationLookup.has(s))
                reservationLookup.set(s, String(rid));
        };
        addKey(rid);
        addKey(r.Id);
    ...
    const orderIdToReservationId = new Map();
    for (const r of reservations) {
        const rid = getReservationKey(r);
        if (!rid)
            continue;
    ...
    for (const so of serviceOrders) {
        const rid = so.ReservationId || so.Reservation?.Id || so.Reservation?.ReservationId || so.Reservation?.ExternalIdentifier;
    ...
    function tryLookupReservationFromItem(it) {
        const candDirect = [
            it.ReservationId,
            it.Reservation?.Id,
        ...
        for (const n of nested) {
            if (!n)
                continue;
            const nN = norm(n);
            if (nN !== null && reservationLookup.has(nN))
                return reservationLookup.get(nN);
        }
        if (it.ExternalIdentifier) {
            const e = norm(it.ExternalIdentifier);
            if (e !== null && reservationLookup.has(e))
                return reservationLookup.get(e);
        }
        ...
        for (const d of dataCandidates) {
            if (!d)
                continue;
            const dN = norm(d);
            if (dN !== null && reservationLookup.has(dN))
                return reservationLookup.get(dN);
        }
    ...
    for (const it of orderItems) {
        ...
        if (foundRid) {
            if (!itemsByReservationId.has(foundRid))
                itemsByReservationId.set(foundRid, []);
    ...
    if (unmatchedItems.length) {
        console.warn(`fetchReservationsForCleaningRange: ${unmatchedItems.length} orderItems could not be linked to reservations for service ${serviceId}. Examples:`);
    ...
    for (const r of reservations) {
        const rid = getReservationKey(r);
        const existingItems = Array.isArray(r.Items)
            ? r.Items
            : Array.isArray(r.OrderItems)
                ? r.OrderItems
                : [];
        const mappedItems = rid && itemsByReservationId.has(rid) ? itemsByReservationId.get(rid) : [];
        let merged = [];
...
async function fetchReservationsForCleaningRange(serviceId, startDate, endDate) {
    ...
    while (cursor.getTime() < endExclusive.getTime()) {
        safety++;
        if (safety > 60)
            throw new Error('fetchReservationsForCleaningRange: safety stop (too many chunks)');
...
function buildLinenCountMapFromOrderItems(items, linenProducts, soToRes) {
    const result = {};
    ...
    const soLookup = (() => {
        if (!soToRes)
            return null;
        if (soToRes instanceof Map)
            return soToRes;
        const m = new Map();
        for (const [k, v] of Object.entries(soToRes))
    ...
    const getProductIdFromItem = (it) => {
        ...
        if (it.Product?.Id)
            return String(it.Product.Id);
        if (it.Product?.ProductId)
            return String(it.Product.ProductId);
        if (it.Data?.Product?.ProductId)
            return String(it.Data.Product.ProductId);
        if (it.Data?.Product?.Id)
            return String(it.Data.Product.Id);
        if (it.Data?.ProductId)
            return String(it.Data.ProductId);
        if (it.Data?.Value?.ProductId)
            return String(it.Data.Value.ProductId);
        if (it.Data?.Value?.Product?.Id)
            return String(it.Data.Value.Product.Id);
        if (it.Data?.Value?.Product?.ProductId)
            return String(it.Data.Value.Product.ProductId);
        ...
        if (value?.Product?.ProductId)
            return String(value.Product.ProductId);
        if (data.ProductId)
    ...
    const resolveReservationId = (it) => {
        const direct = it.ReservationId ||
            it.Reservation?.Id ||
            it.Reservation?.ReservationId ||
            it.Reservation?.ExternalIdentifier;
        if (direct)
            return String(direct);
        const soId = it.ServiceOrderId ||
            it.ServiceOrder?.Id ||
    ...
    for (const it of items || []) {
        ...
        let lp = productIdRaw && linenProducts[productIdRaw.toLowerCase()]
        ...
        if (!lp)
            continue;
        const quantity = Number(it.UnitCount ??
            it.Quantity ??
            it.Count ??
            it.UnitAmount?.Count ??
            it.Data?.UnitCount ??
