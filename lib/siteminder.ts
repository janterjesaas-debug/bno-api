...
const SITEMINDER_CURRENCY =
  (process.env.SITEMINDER_CURRENCY || 'EUR').trim().toUpperCase();
const SITEMINDER_LOCALE =
  (process.env.SITEMINDER_DEFAULT_LOCALE || 'en-US').trim();

export type SiteMinderRoomCategory = {
  Id: string;
  Name?: string;
  Description?: string;
...
export type SiteMinderAvailabilityResult = {
  ResourceCategoryAvailabilities: SiteMinderRoomCategory[];
  raw?: any;
};

export function isSiteMinderConfigured(): boolean {
  return !!(
    SITEMINDER_BASE &&
 ...
 */
export async function fetchSiteMinderAvailability(opts: {
  fromYmd: string;
  toYmd: string;
  adults?: number;
}): Promise<SiteMinderAvailabilityResult> {
 ...
 */
export async function createSiteMinderBooking(opts: {
  fromYmd: string;
  toYmd: string;
  adults: number;
  ...
  guestName?: string;
  guestEmail?: string;
}): Promise<{ ok: boolean; confirmationId?: string | null; raw?: any }> {
  if (!isSiteMinderConfigured()) {
